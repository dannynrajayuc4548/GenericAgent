...
# Force English regardless of system locale - I always want EN prompts/memory templates
...
# Include day of week AND current time in system prompt so the agent can reason about time-sensitive tasks
def get_system_prompt():
    with open(os.path.join(script_dir, f'assets/sys_prompt{lang_suffix}.txt'), 'r', encoding='utf-8') as f: prompt = f.read()
    prompt += f"\nToday: {time.strftime('%Y-%m-%d %a')}  Current time: {time.strftime('%H:%M')} (local)\n"
...
class GeneraticAgent:
    def __init__(self):
        ...
        for k, cfg in mykeys.items():
            ...
            try:
                if 'native' in k and 'claude' in k: llm_sessions += [NativeToolClient(NativeClaudeSess
