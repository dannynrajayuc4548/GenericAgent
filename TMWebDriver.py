...
class Session:
    ...
    def mark_disconnected(self):
        # Only print the disconnect message if the session was previously active
...
class TMWebDriver:  
    def __init__(self, host: str = '127.0.0.1', port: int = 18765):  
        ...
        # Use connect_ex to probe for an existing server; avoids exception on connection refused
    ...
    def start_http_server(self):
        ...
        def long_poll():
            ...
            if session.disconnect_at is not None and session.type != 'http': session.reconnect(queue.Queue(), session_i
